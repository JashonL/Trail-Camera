...
public class IntentUtils {
    ...
    public static void shareFile(Context context, File file, String shareType, String title) {
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.N) {
            if (TYPE_VIDEO.equals(shareType)) {
                ...
                fileUri = FileProvider.getUriForFile(context, CommentUtils.getPackageName(context) + ".fileProvider", file);
            }
    ...
    public static Uri getVideoContentUri(Context context, File file) {
        ...
        if (uri == null) {
            uri = FileProvider.getUriForFile(context, CommentUtils.getPackageName(context) + ".fileProvider", file);
        }
        return uri;
    }
    ...
    public static void shareVideoByUri(Context context, Uri fileUri, String shareType, String title){
        Intent intent = new Intent(Intent.ACTION_SEND);
        intent.putExtra(Intent.EXTRA_STREAM, fileUri);
        intent.setType(shareType);
        Intent chooser = Intent.createChooser(intent, title);
